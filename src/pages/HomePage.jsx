...
const HomePage = () => {
    ...
    return (
        <Fragment>
            ...
            <div className={`w-full z-40 transition-all duration-200 
                ${isSticky ? `sticky lg:top-[5.2rem] md:top-[4.3rem] top-0 bg-white shadow-md` : ""}`}
            >
                <div className="lg:px-14 md:px-8 px-7 flex justify-between items-center 
                    border-b border-[#A6A6A6]"
                >
                    <div className="flex overflow-x-auto no-scrollbar scroll-smooth gap-5 py-3
                        relative w-full" data-aos="zoom-in-up"
                    >
                        {tabs.map((tab) => (
                            <button key={tab.id} ref={(el) => (tabRefs.current[tab.id] = el)}
                                onClick={() => scrollToSection(tab.id)} className={`relative text-sm 
                                md:text-base whitespace-nowrap transition-all duration-200 px-2 py-1 
                                ${activeSection === tab.id ? "text-theme-2 font-semibold" : ""}`}
                            >
                                {tab.label}

                                {activeSection === tab.id && (
                                    <span className="absolute -bottom-3 left-0 right-0 h-[3px] 
                                        bg-[#F36B21] rounded-full"
                                    />
                                )}
                            </button>
                        ))}
